...
const palettes: Record<string, string[]> = {
   ...
   'ace/theme/catppuccin_frappe': ['#f2d5cf', '#eebebe', '#f4b8e4', '#ca9ee6', '#e78284', '#ea999c', '#ef9f76', '#e5c890', '#a6d189', '#81c8be', '#99d1db', '#85c1dc', '#8caaee', '#babbf1', '#c6d0f5', '#b5bfe2', '#a5adce', '#949cbb', '#838ba7', '#737994', '#626880', '#51576d', '#414559', '#303446', '#292c3c', '#232634'],
   'ace/theme/catppuccin_macchiato': ['#f4dbd6', '#f0c6c6', '#f5bde6', '#c6a0f6', '#ed8796', '#ee99a0', '#f5a97f', '#eed49f', '#a6da95', '#8bd5ca', '#91d7e3', '#7dc4e4', '#8aadf4', '#b7bdf8', '#cad3f5', '#b8c0e0', '#a5adcb', '#939ab7', '#8087a2', '#6e738d', '#5b6078', '#494d64', '#363a4f', '#24273a', '#1e2030', '#181926'],
...
describe('catppuccin editor themes', () => {
   test('the tables in this file cover exactly the flavours the theme file registers', () => {
      const registeredIds = Object.keys(localThemes()).sort();

      assert.deepEqual(Object.keys(catppuccinFlavours).map(code => `ace/theme/${code}`).sort(), registeredIds);
      assert.deepEqual(Object.keys(palettes).sort(), registeredIds);
   });

   test('every flavour is offered in the picker, in the group that matches their brightness', () => {
      const { light, dark } = offeredThemesByGroup();
